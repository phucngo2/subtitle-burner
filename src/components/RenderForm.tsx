...
import { generateVideoExtension, getFileValidator } from "@/utils";
...
export const RenderForm: Component<{}> = () => {
  ...
  const inputVideo = form.useStore((state) => state.values.inputVideo);
  ...
  const extension = () => {
    const ext = generateVideoExtension(inputVideo());
    return ext ? `.${ext}` : "";
  };
      ...
      <form.Field name="outputVideo">
        {(field) => (
          <Input
            ...
            Icon={<SaveIcon />}
            badge={extension()}
            placeholder="Enter output file name (Optional)"
          />
        )}
      </form.Field>
